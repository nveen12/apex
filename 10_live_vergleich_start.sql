...
begin
    ...
    wwv_flow_imp_page.create_report_region(
        p_id                         => wwv_flow_imp.id(91000000000000016),
        p_name                       => 'DB-Link TODOs',
        p_title                      => 'Nicht vergleichbar - DB-Link fehlt',
        ...
        p_source                     => wwv_flow_string.join(wwv_flow_t_varchar2(
            'select fv.fv_name          as "Fachverfahren",',
            '       tp.service_name     as "Service",',
            '       src.hostname        as "Quelle Host",',
            '       tgt.hostname        as "Ziel Host",',
            '       sp.dblink_name      as "Quelle DB-Link",',
            '       tp.dblink_name      as "Ziel DB-Link"',
            'from   mt_fachverfahren fv',
            'join   mt_fv_pdb_mapping tm on tm.fv_id = fv.fv_id',
            'join   mt_pdb tp            on tp.pdb_id = tm.pdb_id',
            'join   mt_cdb tc            on tc.cdb_id = tp.cdb_id',
    ...
    wwv_flow_imp_page.create_page_process(
        ...
        p_process_sql_clob       => wwv_flow_string.join(wwv_flow_t_varchar2(
            'begin',
            '    -- Default for migration tracking: compare the selected Fachverfahren schema.',
            '    -- Whole-service comparison remains possible by calling Page 9 with',
            '    -- P9_SCHEMA_NAME = __ALL_APP_SCHEMAS__.',
            '    select fv.fv_id, nvl(fv.schema_name, ''__ALL_APP_SCHEMAS__'')',
