...
begin
    ...
    wwv_flow_imp_page.create_page_item(
        ...
        p_lov                   => wwv_flow_string.join(wwv_flow_t_varchar2(
            'select nvl(tp.service_name, tp.pdb_name)',
            '       || '' | '' || listagg(distinct src.hostname, '' + '')',
            '          within group (order by src.hostname)',
            '       || '' --> '' || tgt.hostname as display_value,',
            ...
            'group  by tm.mapping_id, nvl(tp.service_name, tp.pdb_name), tgt.hostname',
            'order  by nvl(tp.service_name, tp.pdb_name), tgt.hostname')),
